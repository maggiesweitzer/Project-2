...
For monsters: trait = strength, intelligence, charisma, dexterity
For spells: group = level or components
...
class_tbl <- dnd_spells_tbl |>
  ...
  ggplot(data = dnd_spells_tbl |>
         group_by(spell_level) |>
         summarize(across(where(is.logical),
                          list("sum" = ~ sum(.x, na.rm = TRUE)), .names = "{.col}"))) +
  geom_bar(stat = "identity") +
    aes(x = "spell_level", y = "count()")

  
  ggplot(data = class_tbl |>
           select("spell_level", "Bard")) +
    aes(x = as.factor(spell_level), y = count) +
    geom_bar(stat = "identity")
  
  
  cor_val <- dnd_data_tbl |>
         summarize(cor_val = cor(spell_level, target_range_sort))
    #pull(cor_val) |>
       paste0("The correlation between spell_level and target_range_sort is ", round(cor_val, 2))
```
